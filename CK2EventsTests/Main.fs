...
let parseEqualityCheck file =
    ...
    let parsedAgain = CKParser.parseEventString pretty "test"
...
let parserTests =
    testList "parser tests" [
        ...
        testCase "bool test" <| fun () ->
            let parsed = (CKParser.parseEventFile "CK2EventsTests/event test files/bool.txt")
            match parsed with
                | Success(v,_,_) ->
                    let target = (ParsedFile [KeyValue(PosKeyValue(range.Zero, KeyValueItem(Key("test"),Bool(true))))])
...
let test file =
    ...
    |Success(v, _, _) ->
        ...
        let rawAgain = processed.ToRaw |> ParsedFile
...
let processingTests =
    testList "processing tests" [
        ...
        testCase "addLocalisation" <| fun () ->
            let parsed = CKParser.parseEventString "character_event = { desc = LOCTEST }" "test"
